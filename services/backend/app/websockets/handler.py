...
from app.generation.unified_flow import generate_unified_flow  # NEW: Unified flow generation
from app.generation.agentic_orchestrator import AgenticOrchestrator  # AGENTIC: Agent-based generation
...
async def handle_generate_flow(websocket: WebSocket, data: Dict[str, Any], user_id: str):
    ...
    try:
        ...
        screen_metadata = unified_result['screens']
        
        print(f"\n✅ Unified flow complete!")
        print(f"  Project files: {len(project['files'])}")
        print(f"  Screens: {len(screen_metadata)}")
        
        # Build flow_graph structure for database/frontend
        flow_graph = {
...
async def handle_generate_flow_agentic(websocket: WebSocket, data: Dict[str, Any], user_id: str):
    """
    Handle agentic flow generation with progressive building
    
    Uses agent loop with tool use to:
    - Define architecture
    - Build files progressively
    ...
    try:
        ...
        print(f"\n{'='*80}")
        print(f"AGENTIC FLOW GENERATION")
        print(f"{'='*80}")
        print(f"Project ID: {project_id}")
        print(f"Task: {task_description[:100]}...")
        print(f"Taste ID: {selected_taste_id}")
        ...
        # Load DTM
        await send_progress(websocket, "loading_dtm", "Loading designer taste...")
        ...
        if selected_taste_id:
            try:
                ...
                # Format taste context for agent
                taste_context = {
                    "exact_tokens": dtm.get("exact_tokens", ""),
                    "patterns": dtm.get("patterns", ""),
                    "personality": dtm.get("personality", ""),
                    "examples": dtm.get("examples", "")
                }
        ...
        # Initialize agent callbacks
        architecture_sent = False
        files_sent = set()
        
        async def on_file_update(path: str, content: str):
            """Stream file updates to frontend"""
            files_sent.add(path)
            await websocket.send_json({
                "type": "file_update",
                "data": {
        ...
        async def on_architecture(arch: Dict[str, Any]):
            """Stream architecture definition to frontend"""
            nonlocal architecture_sent
            
            if not architecture_sent:
                architecture_sent = True
                
                # Calculate layout positions
                layout_positions = calculate_layout_positions(arch)
                
                # Send architecture to frontend
                await websocket.send_json({
                    "type": "flow_architecture",
                    "data": {
                        "flow_id": f"flow_{project_id[:8]}",
                        "flow_name": f"Flow for {task_description[:30]}...",
                        "description": task_description,
                        "entry_screen_id": arch["entry_screen_id"],
                        "screens": [
                            {
                                "screen_id": s["screen_id"],
                                "name": s["name"],
        ...
        await send_progress(websocket, "generating", "Starting agentic generation...")
        ...
        orchestrator = AgenticOrchestrator(
            ...
            on_architecture=on_architecture
        )
        ...
        print(f"✅ Agentic generation complete!")
        print(f"  Files generated: {len(result['files'])}")
        print(f"  Architecture screens: {len(result.get('architecture', {}).get('screens', []))}")
        ...
        flow_graph = {
            "flow_id": f"flow_{project_id[:8]}",
            "flow_name": f"Flow for {task_description[:50]}",
            "display_title": task_description[:50],
            "display_description": task_description,
            "entry_screen_id": architecture.get("entry_screen_id"),
            "project": {
                "files": result["files"],
            ...
            "layout_positions": calculate_layout_positions(architecture) if architecture else {},
            "layout_algorithm": "hierarchical"
        }
        ...
        print(f"✅ Complete! Version {version} saved")
        print(f"{'='*80}\n")
        ...
        await websocket.send_json({
            ...
            "result": {
                "status": "success",
                "version": version,
                "flow_graph": flow_graph_for_client
            }
        })
        
    except Exception as e:
        print(f"\n❌ Error in agentic generation: {e}")
        import traceback
        traceback.print_exc()
        await send_error(websocket, f"Generation failed: {str(e)}")
    ...
    try:
        while True:
            ...
            if action == "build-dtr":
                ...
                await handle_generate_flow_agentic(websocket, data, user_id)
