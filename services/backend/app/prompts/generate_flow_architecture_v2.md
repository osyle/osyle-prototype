...
2. **Respect their intent**:
   ...
   - **Exact mode screens**: These are screens user wants recreated with specific content/layout - honor that intent
   - **Inspiration mode screens**: These are looser suggestions - you can interpret more freely
...
- **reference_mode**: If `user_provided` is true, either "exact" or "inspiration" based on user's mode. `null` if screen was generated by you.
...
4. **Respect exact mode**:
   - Screens marked "exact" have specific designs attached - your task_description should note this
   - Example: "Recreate the login form from user's design with email, password fields, and submit button"
5. **Interpret inspiration mode**:
   - Screens marked "inspiration" are looser - use as content hints
   - Example: "Product grid page inspired by user's reference images"
6. **Sequential ordering**: User's screen definitions are in order - maintain that sequence
  ...
  "screens": [
    {
      ...
      "task_description": "Recreate user's exact design showing ingredient list with checkboxes, quantities, and 'Start Cooking' button",
      "platform": "phone",
...
10. **Screen count based on TASK complexity, not screen definitions**: Whether or not user provided screen definitions does NOT determine single vs multi-screen. That decision is based purely on the task:
