...
import ScreenEdge from '../concept/ScreenEdge'
import type { ScreenEdgeData } from '../concept/ScreenEdge'
import ScreenNode from '../concept/ScreenNode'
import type { ScreenNodeData } from '../concept/ScreenNode'
...
export default function ReactFlowCanvas({
  ...
  // Highway Y: 220px above the topmost screen.
  // This clears the resize panel (-160px) and screen name label (-28px)
  // with room to spare for the descending-segment labels.
  const highwayY = useMemo(() => {
    if (screenPositions.size === 0) return -120
    const minY = Math.min(...Array.from(screenPositions.values()).map(p => p.y))
    return minY - 220
  }, [screenPositions])

  // All possible edges — not yet filtered
  const allEdges: Edge[] = useMemo(
    () =>
      flowGraph.transitions.map(transition => ({
        ...
        data: {
          flowType: transition.flow_type,
          highwayY,
        } as ScreenEdgeData,
      })),
    [flowGraph.transitions, highwayY],
  )

  // Only show outgoing edges from the selected screen —
  // keeps the canvas clean and labels readable
  const visibleEdges: Edge[] = useMemo(() => {
    if (!selectedScreenId) return []
    return allEdges.filter(e => e.source === selectedScreenId)
  }, [allEdges, selectedScreenId])
