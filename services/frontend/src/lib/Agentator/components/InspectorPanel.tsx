...
export const InspectorPanel: React.FC<InspectorPanelProps> = ({
  ...
  const panelClass = isTop
    ? 'agentator-inspector-top'
    : 'agentator-inspector-bottom'

  return (
    <div className={`agentator-inspector ${panelClass}`} data-annotation-ui>
      <div
        className="agentator-inspector-header"
      ...
      <div className="agentator-inspector-content">
        <div className="agentator-inspector-section">
          ...
          <div
            className="agentator-inspector-value"
            title={inspectedElement.element}
          >
        ...
        <div className="agentator-inspector-section">
          ...
          <div
            className="agentator-inspector-value agentator-inspector-path"
            title={inspectedElement.elementPath}
          >
        ...
        {inspectedElement.boundingBox && (
          <div className="agentator-inspector-section">
            ...
            <div className="agentator-inspector-value agentator-inspector-position">
              {Math.round(inspectedElement.boundingBox.x)}px,{' '}
              {Math.round(inspectedElement.boundingBox.y)}px
              <span className="agentator-inspector-dim">
                ({Math.round(inspectedElement.boundingBox.width)}×
                {Math.round(inspectedElement.boundingBox.height)}px)
              </span>
            </div>
          </div>
        )}
                ...
                {Object.entries(inspectedElement.attributes)
                  ...
                  .map(([key, value]) => (
                    <div key={key} className="agentator-inspector-attribute">
                      <span className="agentator-inspector-attr-key">
                        {key}:
                      </span>
                      <span
                        className="agentator-inspector-attr-value"
                        title={value}
                      >
